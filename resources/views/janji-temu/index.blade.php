    ...
    <div class="grid lg:grid-cols-3 gap-8">
        <div class="lg:col-span-1">
            <div class="bg-white rounded-xl shadow-lg border border-gray-100 p-6">
                ...
                <form action="{{ route('janji-temu.store') }}" method="POST" class="space-y-4">
                    @csrf
                    <div>
                        <label for="date" class="block text-sm font-semibold text-navy mb-2">Tanggal</label>
                        <input type="date" id="date" name="date" required class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-navy focus:ring-2 focus:ring-navy/20">
                    </div>
                    <div>
                        <label for="purpose" class="block text-sm font-semibold text-navy mb-2">Keperluan</label>
                        <textarea id="purpose" name="purpose" rows="4" required class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-navy focus:ring-2 focus:ring-navy/20" placeholder="Jelaskan keperluan Anda"></textarea>
                    </div>
                    <button type="submit" class="w-full bg-gradient-to-r from-navy to-navy-dark text-white py-3 rounded-xl font-semibold hover:from-navy-dark hover:to-navy">
                        Kirim Permintaan
                    </button>
                </form>
            </div>
                        ...
                        <tbody class="divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <form action="{{ route('janji-temu.destroy', $item) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin membatalkan janji temu ini?');">
                                        @csrf
                                        @method('DELETE')
